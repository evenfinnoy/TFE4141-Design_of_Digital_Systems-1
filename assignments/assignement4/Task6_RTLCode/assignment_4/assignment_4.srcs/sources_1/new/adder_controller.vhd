...
entity adder_controller is
  port (
    ...
    -- Control signals for computation interface
...
begin
  ...
  if(reset_n='0') then
    ...
    -- b3 has just been accepted, so the complete result
    -- is now stored in y_r
    if(input_reg_en_i = '1' and input_shift_counter_r = 7) then
      result_ready_r <= '1';

    -- Clear the initial ready flag once the first output
